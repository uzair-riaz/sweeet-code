  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 bg-card border-t border-border">
      <div className="flex items-center justify-between p-4 border-b border-border">
        <h3 className="font-medium">{mode === 'run' ? 'Execution Result' : 'Test Results'}</h3>
            ...
            <div className="flex items-center justify-between">
              ...
              {result.execution_time && (
                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                  ...
                  {result.memory_used && <span>{(result.memory_used / 1024).toFixed(2)}MB</span>}
                </div>
              )}
            </div>
            <div className="bg-muted/30 rounded-lg p-3 space-y-2 text-sm font-mono">
              ...
              {result.output && (
                <div>
                  ...
                  <span className={!result.passed ? 'text-red-500' : ''}>{result.output}</span>
                </div>
              )}
